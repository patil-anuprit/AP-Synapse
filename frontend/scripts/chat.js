...
// ============================================================
// AP_PRODUCTION_BACKEND_FAILOVER_V3
// Render primary + Railway independent secondary
// ============================================================
...
const AP_BACKEND_FAILOVER_HTTP =
    new Set([
...
/*
 * If the primary fails, use the secondary first for a short
 * cooldown. Afterwards AP Synapse automatically probes the
 * primary again, so recovery requires no user action.
 */
let apPrimaryRetryAfter = 0;

function apBackendCandidates(url) {
...
async function apResilientFetch(
    ...
    /*
     * Three rounds × two independent backends.
     * A failed primary immediately falls through to Railway;
     * we do not wait for the next retry round.
     */
    const maxRounds = 3;
        ...
        for (
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    apMarkBackendSuccess(
                        endpoint
                    );

                    console.log(
                        "AP Synapse backend active:",
                        endpoint.startsWith(
                            AP_PRIMARY_API_BASE
                        )
                            ? "primary"
                ...
                if (
                    !AP_BACKEND_FAILOVER_HTTP
                        .has(
                            response.status
                        )
                ) {
                    /*
                     * 400 / 401 / 403 etc. are real request or
                     * authorization errors. Switching hosts
                     * would not correctly fix those.
                     */
                    return response;
                }
            ...
            /*
             * Inform the user only when the primary path
             * actually failed and AP Synapse is switching.
             */
            if (
                !fallbackNoticeShown &&
                endpoint.startsWith(
                ...
                if (
                    ...
                    showToast(
                        "Switching to AP Synapse backup connection..."
                    );
                }

                console.warn(
                    "AP Synapse primary backend unavailable. Trying secondary."
                );
            }
        }
    ...
    throw (
        lastError ||
        new Error(
            "AP Synapse backend recovery exhausted."
        )
    );
}
